...
data.iGABA.voom <- voom(counts=data.dge.iGABA, design=design_matrix_iGABA, plot=TRUE) 
data.iGlut.voom <- voom(counts=data.dge.iGlut, design=design_matrix_iGlut, plot=TRUE) 
data.iAstro.voom <- voom(counts=data.dge.iAstro, design=design_matrix_iAstro, plot=TRUE) 
...
meta.iGABA <- read.csv('results/SVA/iGABA_meta_for_SVA.csv', header = TRUE)
meta.iGlut <- read.csv('results/SVA/iGlut_meta_for_SVA.csv')
meta.iAstro <- read.csv('results/SVA/iAstro_meta_for_SVA.csv')

dge.voom.iGABA <- read.csv('data/iGABA_voom_DGE.csv')
dge.voom.iGlut <- read.csv('data/iGlut_voom_DGE.csv')
dge.voom.iAstro <- read.csv('data/iAstro_voom_DGE.csv')

gene_counts_aftercpm.df.iAstro <- read.delim('gene_counts_aftercpm.df.iAstro.xls', header = TRUE, sep='\t')
gene_counts_aftercpm.df.iGABA <- read.delim('gene_counts_aftercpm.df.iGABA.xls', header = TRUE, sep='\t')
gene_counts_aftercpm.df.iGlut <- read.delim('gene_counts_aftercpm.df.iGlut.xls', header = TRUE, sep='\t')
...
### change meta variables to numeric 
meta.iAstro <- meta.iAstro %>%
  mutate(Treatment = as.character(Treatment),
         Treatment = case_when(
           ...
           Treatment == 'lep'      ~ '2',
           Treatment == 'sema'     ~ '3',
           Treatment == 'ghreSema' ~ '4',
           Treatment == 'lepSema'  ~ '5'
         )) %>%
  mutate(Treatment = as.numeric(Treatment))

meta.iGlut <- meta.iGlut %>%
  mutate(Treatment = as.character(Treatment),
         Treatment = case_when(
           Treatment == 'veh'      ~ '0',
           Treatment == 'ghre'     ~ '1',
...
meta.iAstro <- subset(metadata.df.iAstro, select=c('sampleName', 'Treatment', 'Well', 'SV1'))
...
data.iGABA.voom <- data.iGABA.voom[ , -c(1, 2, 3, 4, 5)]
data.iGlut.voom <- data.iGlut.voom[ , -c(1, 2, 3, 4, 5)]
data.iAstro.voom <- data.iAstro.voom[ , -c(1, 2, 3, 4, 5)]


# model (all vars are categorical --> modeled as random effects), hardcode in SVs 
form.iAstro <- ~ Treatment + Well + SV1
...
```{r}
# --------------------------------------------------------------------------- DESIGN MATRIX w/ SVs
# add donor back into meta
meta.iGABA <- subset(metadata.df.iGABA, select=c('sampleName', 'Treatment', 'Well', 'SV1','SV2', 'SV3', 'Donor'))
meta.iGlut <- subset(metadata.df.iGlut, select=c('sampleName', 'Treatment', 'Well','SV1','Donor'))
meta.iAstro <- subset(metadata.df.iAstro, select=c('sampleName', 'Treatment', 'Well', 'SV1'))



meta.iGABA$Treatment <- factor(meta.iGABA$Treatment)
meta.iGlut$Treatment <- factor(meta.iGlut$Treatment)
meta.iAstro$Treatment <- factor(meta.iAstro$Treatment)

# refactor reference level (to compare against veh)
meta.iGABA$Treatment <- relevel(meta.iGABA$Treatment, ref = '0')
meta.iGlut$Treatment <- relevel(meta.iGlut$Treatment, ref = '0')
meta.iAstro$Treatment <- relevel(meta.iAstro$Treatment, ref = '0')
...
# remove donor from design matrix
  # use dup.cor() from limma to account for repeated measure donor effects
design_matrix_iGABA <- model.matrix( ~ 0 + Treatment + SV1 + SV2 + SV3 + Well , data = meta.iGABA) 
design_matrix_iGlut <- model.matrix(~ 0 + Treatment  + SV1 + Well , data = meta.iGlut) 
design_matrix_iAstro <- model.matrix(~ 0 + Treatment + SV1 + Well, data = meta.iAstro) 
```
...
# duplicate corr 
```{r}
dupCorr_iGlut <-duplicateCorrelation(data.iGlut.voom, design_matrix_iGlut, block=meta.iGlut$Donor)
dupCorr_iGABA <-duplicateCorrelation(data.iGABA.voom, design_matrix_iGABA, block=meta.iGABA$Donor)
# didn't do for Astro - only have 1 donor! 

cat(dupCorr_iGABA$consensus.correlation) # 0.9715695
cat(dupCorr_iGlut$consensus.correlation) # 0.9894474

as.data.frame(dupCorr_iGABA)
as.data.frame(dupCorr_iGlut)

PATH <- sprintf('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/results')
write_csv(dupCorr_iGABA, file=paste(PATH, '/dupCorr_iGABA.txt', delim ) )
write_csv(dupCorr_iGlut, file=paste(PATH, '/dupCorr_iGlut.txt' ) )
...
# decide if doing 1st or 2nd way 
# jasmine does VP w/ corrected expression
  #use normalized counts
  # add covariates to include (which is decided by VP) into a new design matrix
  # use this DM to do lm fit 
