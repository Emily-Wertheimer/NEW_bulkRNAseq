...
# ADD SVs HERE FOR SVs NOT EXPLAINED BY META
...
# --------------------------------------------------------------------------- DESIGN MATRIX W/ SVs
# redo design matrix w/ sv
setwd('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG')

metadata.df.iGABA <- read.delim('iGABA_SVA_meta_w_sv.csv', sep = ',')
metadata.df.iAstro <- read.delim('iAstro_SVA_meta_w_sv.csv', sep = ',')
metadata.df.iGlut <- read.delim('iAstro_SVA_meta_w_sv.csv', sep = ',')

# ADD SVs HERE FOR SVs NOT EXPLAINED BY META
design_matrix_iGABA <- model.matrix( ~ 0 + Treatment + SV1 + SV2 + SV3, data = metadata.df.iGABA) 
design_matrix_iGlut <- model.matrix(~ 0 + Treatment  + SV1, data = metadata.df.iGlut) 
design_matrix_iAstro <- model.matrix(~ 0 + Treatment + SV1, data = metadata.df.iAstro) 
```
...
data.iGABA.voom <- read.delim('iGABA_voom_DGE.csv', header=TRUE)
data.iGlut.voom <- read.delim('iGlut_voom_DGE.csv', header=TRUE)
data.iAstro.voom <- read.delim('iAstro_voom_DGE.csv', header=TRUE)
...
# load corr outfiles from SVA
setwd('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/results/SVA/')

iGABA_cor_mat <- read.delim('iGABA_SVA_correlation_results.csv', header=TRUE, sep=',')
iGlut_cor_mat <- read.delim('iGlut_SVA_correlation_results.csv', header=TRUE, sep=',')
iAstro_cor_mat <- read.delim('iAstro_SVA_correlation_results.csv', header=TRUE, sep=',')

# print unique nonSig SVs for e/ cell type
setwd('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/data/')
iGlut_SV <- read.delim('iGlut_nonSigSVs.txt')
iGABA_SV <- read.delim('iGABA_nonSigSVs.txt')
iAstro_SV <- read.delim('iAstro_nonSigSVs.txt')


iGlut_SV_preserve <- unique(iGlut_SV$SV_Var)
iGABA_SV_preserve <- unique(iGABA_SV$SV_Var)
iAstro_SV_preserve <- unique(iAstro_SV$SV_Var)

cat(iGlut_SV_preserve)
cat(iGABA_SV_preserve)
cat(iAstro_SV_preserve)
...
form.iAstro <- ~ (1|Donor) + (1|Treatment) + (1|Well) + (1|SV1)
form.iGlut <- ~ (1|Donor) + (1|Treatment) + (1|Well) + (1|SV1) + (1|SV2)
form.iGABA <- ~ (1|Donor) + (1|Treatment) + (1|Well) + (1|SV1) + (1|SV2) + (1|SV3)
...
varPart.iGABA <- fitExtractVarPartModel(data.iGABA.voom, form.iGABA, metadata.df.iGABA)
varPart.iGlut <- fitExtractVarPartModel(data.iGlut.voom, form.iGlut, metadata.df.iGlut)
varPart.iAstro <- fitExtractVarPartModel(data.iAstro.voom, form.iAstro, metadata.df.iAstro) ##### 3/18 STOPPED HERE: Error in filterInputData(exprObj, formula, data, useWeights = useWeights) : 
...
## ADD SVS NOT EXPLAINED BY META SOMEWHERE AROUND HERE ^^
```
