...
design_matrix_iAstro_sv <- model.matrix(~ 0 + Treatment + SV1 + Well, data = meta.iAstro) 
```
...
setwd('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/data/')
data.iGABA.voom <- read.delim('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/data/iGABA_voom_DGE.csv', header=TRUE,sep=",")# row.names=1, )
data.iGlut.voom <- read.delim('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/data/iGlut_voom_DGE.csv', header=TRUE,sep=",")#row.names=1, sep=",")
data.iAstro.voom <- read.delim('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/data/iAstro_voom_DGE.csv', header=TRUE,sep=",")#row.names=1, sep=",")
...
## DEBUG:  can't pass non-numeric expression object to lm fit
...
View(data.iAstro.voom[missing_ensembl.iAstro, ])
...
data.fit.iAstro <- lmFit(object=data.iAstro.voom, design=design_matrix_iAstro_sv) # coefficients not estimable: WellC1 WellC4. Warning: Partial NA coefficients for 14181 probe(s)
...
# --------------------------------------------------------------------------- ENHANCED VOLCANO
# test w/ iAstro
results.iAstro.ghre <- read.delim('/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/results/DEG/DEG_response_iAstro_ghre_v_veh.txt')

# plot only unique genes (duplicated genes were filtered for the appearance w/ the lowest adjust pval)
to.plot <- results.iAstro.ghre %>%
  group_by(Gene_name.x) %>%
  slice_min(order_by = adj.P.Val, n = 1) %>%
  ungroup()

geneNames <- to.plot$Gene_names.x



#   # Generate the plot
#   volcanoPlot <- EnhancedVolcano(to.plot,
#                                  lab = rownames(to.plot),
#                                  x = 'logFC',
#                                  y = 'adj.P.Val',
#                                  pCutoff = 0.05,
#                                  FCcutoff = 0.5,
#                                  ylim = c(0, 30),
#                                  xlim = c(-5, 5),
#                                  col = c('grey', 'lightskyblue', 'lightblue', 'red'))
#   
#   figuresPath <- '/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/figures/'
#   setwd(figuresPath)
#   
#   # Construct output file name
#   outfile <- paste0(figuresPath, resultsFileName, '_EnhancedVolcano.png')
#   
#   # Save plot
#   ggsave(outfile, plot = volcanoPlot, width = 10, height = 8, dpi = 300)
# }
# 
# PATH = '/gpfs/gibbs/pi/huckins/ekw28/bulkRNAseq/ghre_lep_sema_RNAseq/3_DEG/results/DEG/'
# enhancedVolcanoFx('DEG_response_iAstro_ghre_v_veh', PATH)
